...
def save_code(request):
    if request.method == "POST":
        ...
        if pk:
            # 编辑已有脚本
            playbook = get_object_or_404(PlaybookCode, pk=pk, created_by=request.user)

            # 检查是否改名后与其他脚本冲突（允许保留原名）
            if PlaybookCode.objects.filter(created_by=request.user, name=code_name).exclude(pk=pk).exists():
                return JsonResponse({
                    'status': 'error',
...
def get_playbook_list_api(request):
    ...
    # 获取当前用户的 playbook 列表
        playbooks = PlaybookCode.objects.filter(query,created_by=request.user).order_by('-created_at')
...
def batch_delete(request):
    ...
    try:
        ...
        # 获取当前用户有权删除的主机
        playbooks = PlaybookCode.objects.filter(
            id__in=playbook_ids,
